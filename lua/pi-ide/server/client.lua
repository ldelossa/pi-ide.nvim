...
local frame = require("pi-ide.server.frame")
local handshake = require("pi-ide.server.handshake")
...
function M.process_data(client, data, on_message, on_close, on_error, auth_token)
  ...
  if not client.handshake_complete then
    ...
    if complete and request then
      local success, response_from_handshake, _, error_message = handshake.process_handshake(request, auth_token)
      if success then
        logger.debug("client", "handshake ok:", client.id)
      else
        logger.warn("client", "handshake failed:", client.id, error_message or "unknown")
      end

      client.tcp_handle:write(response_from_handshake, function(err)
        if err then
          logger.error("client", "failed to send handshake response:", client.id, err)
        ...
        if success then
          ...
          vim.schedule(function() client.tcp_handle:close() end)
        end
      end)
    end
    return
  ...
  while #client.buffer >= 2 do
    ...
    if parsed_frame.opcode == frame.OPCODE.TEXT or parsed_frame.opcode == frame.OPCODE.BINARY then
      vim.schedule(function() on_message(client, parsed_frame.payload) end)
