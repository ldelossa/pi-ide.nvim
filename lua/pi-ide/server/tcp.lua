...
local client_manager = require("pi-ide.server.client")
...
---Find an available port by random sampling and probing.
---@param min_port number Minimum port to try
---@param max_port number Maximum port to try
---@param max_attempts number|nil Maximum probe attempts (default 32)
---@return number|nil port Available port number, or nil if none found
function M.find_available_port(min_port, max_port, max_attempts)
  if min_port > max_port then return nil end
  max_attempts = max_attempts or 32
  local span = max_port - min_port + 1
  -- libuv defers bind to listen, so bind alone reports success even when the
  -- port is taken; we must probe with listen as well.
  for _ = 1, max_attempts do
    local port = min_port + math.random(0, span - 1)
    local test_server = vim.loop.new_tcp()
    if test_server then
      local bind_ok = test_server:bind("127.0.0.1", port)
      local listen_ok = bind_ok and test_server:listen(1, function() end)
      test_server:close()
      if bind_ok and listen_ok then return port end
    end
  end
  return nil
end
...
function M._disconnect_client(server, client, code, reason)
  ...
  if not server.clients[client.id] then return end
